...
import org.cryptobiotic.protoconvert.import
...
actual class ConsumerProto actual constructor(val groupContext: GroupContext, val topDir: String): Consumer {
    ...
    actual override fun readEncryptedBallotChain(device: String): Result<EncryptedBallotChain, ErrorMessages> {
        ...
        return try {
            var proto: org.cryptobiotic.protogen.EncryptedBallotChain
            FileInputStream(ballotChainFile).use { inp ->
                proto = org.cryptobiotic.protogen.EncryptedBallotChain.decodeFromStream(inp)
            }
    ...
    private fun makeManifestResult(manifestBytes: ByteArray): Result<Manifest, ErrorMessages> {
        return try {
            var proto: org.cryptobiotic.protogen.Manifest
            ByteArrayInputStream(manifestBytes).use { inp ->
                proto = org.cryptobiotic.protogen.Manifest.decodeFromStream(inp)
            }
            Ok(proto.import())
        } catch (t: Throwable) {
    ...
    private fun readElectionConfig(filename: String): Result<ElectionConfig, ErrorMessages> {
        ...
        return try {
            var proto: org.cryptobiotic.protogen.ElectionConfig
            FileInputStream(filename).use { inp -> proto = org.cryptobiotic.protogen.ElectionConfig.decodeFromStream(inp) }
            proto.import(errs)
        } catch (t: Throwable) {
            errs.add("Exception= ${t.message} ${t.stackTraceToString()}")
        }
    ...
    private fun GroupContext.readElectionInitialized(filename: String): Result<ElectionInitialized, ErrorMessages> {
        ...
        return try {
            FileInputStream(filename).use { inp ->
                val proto = org.cryptobiotic.protogen.ElectionInitialized.decodeFromStream(inp)
    ...
    private fun GroupContext.readTallyResult(filename: String): Result<TallyResult, ErrorMessages> {
        ...
        return try {
            var proto: org.cryptobiotic.protogen.TallyResult
            FileInputStream(filename).use { inp ->
                proto = org.cryptobiotic.protogen.TallyResult.decodeFromStream(inp)
            }
    ...
    private fun GroupContext.readDecryptionResult(filename: String): Result<DecryptionResult, ErrorMessages> {
        ...
        return try {
            var proto: org.cryptobiotic.protogen.DecryptionResult
            FileInputStream(filename).use { inp ->
                proto = org.cryptobiotic.protogen.DecryptionResult.decodeFromStream(inp)
            }
            val result = proto.import(this, errs)
            if (errs.hasErrors()) Err(errs) else Ok(result!!)
        } catch (t: Throwable) {
    ...
    private fun GroupContext.readTrustee(filename: String): Result<DecryptingTrusteeIF, ErrorMessages> {
        ...
        return try {
            var proto: org.cryptobiotic.protogen.DecryptingTrustee
            FileInputStream(filename).use { inp ->
                proto = org.cryptobiotic.protogen.DecryptingTrustee.decodeFromStream(inp)
            }
            val result = proto.import(this, errs)
            if (errs.hasErrors()) Err(errs) else Ok(result!!)
        } catch (t: Throwable) {
    ...
    private class PlaintextBallotIterator(
        ...
        override fun computeNext() {
            while (true) {
                ...
                val ballotProto = org.cryptobiotic.protogen.PlaintextBallot.decodeFromByteBuffer(ByteBuffer.wrap(message))
    ...
    private class EncryptedBallotBatchIterator(
        ...
        private val protoFilter: Predicate<org.cryptobiotic.protogen.EncryptedBallot>?,
        private val filter: Predicate<EncryptedBallot>?,
    ) : AbstractIterator<EncryptedBallot>() {
        ...
        override fun computeNext() {
            while (true) {
                ...
                val message = input.readNBytes(length)
                val ballotProto = org.cryptobiotic.protogen.EncryptedBallot.decodeFromByteBuffer(ByteBuffer.wrap(message))
    ...
    private fun readEncryptedBallot(ballotFile : String, errs: ErrorMessages): EncryptedBallot? {
        FileInputStream(ballotFile).use { inp ->
            val proto = org.cryptobiotic.protogen.EncryptedBallot.decodeFromStream(inp)
    ...
    private inner class DecryptedBallotBatchIterator(filename: String) : AbstractIterator<DecryptedTallyOrBallot>() {
        ...
        override fun computeNext() {
            while (true) {
                ...
                val message = input.readNBytes(length)
                val proto = org.cryptobiotic.protogen.DecryptedTallyOrBallot.decodeFromByteBuffer(ByteBuffer.wrap(message))
    ...
    private inner class DecryptedBallotFileIterator(ballotDir: Path) : AbstractIterator<DecryptedTallyOrBallot>() {
        ...
        override fun computeNext() {
            while (idx < pathList.size) {
                val ballotFile = pathList[idx++]
                var proto: org.cryptobiotic.protogen.DecryptedTallyOrBallot
                FileInputStream(ballotFile.toString()).use { inp ->
                    proto = org.cryptobiotic.protogen.DecryptedTallyOrBallot.decodeFromStream(inp)
                }
