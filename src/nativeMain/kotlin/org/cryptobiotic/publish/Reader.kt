...
fun makeManifestInternal(manifestBytes: ByteArray): Result<Manifest, String> {
    val proto = org.cryptobiotic.protogen.Manifest.decodeFromByteArray(manifestBytes)
    return proto.import()
}

fun readElectionConfig(filename: String): Result<ElectionConfig, String> {
    val buffer = gulp(filename)
    val proto = org.cryptobiotic.protogen.ElectionConfig.decodeFromByteArray(buffer)
    return proto.import()
}

fun GroupContext.readElectionInitialized(filename: String): Result<ElectionInitialized, String> {
    val buffer = gulp(filename)
    val proto = org.cryptobiotic.protogen.ElectionInitialized.decodeFromByteArray(buffer)
    return proto.import(this)
}

fun GroupContext.readTallyResult(filename: String): Result<TallyResult, String> {
    val buffer = gulp(filename)
    val proto = org.cryptobiotic.protogen.TallyResult.decodeFromByteArray(buffer)
    return proto.import(this)
}

fun GroupContext.readDecryptionResult(filename: String): Result<DecryptionResult, String> {
    val buffer = gulp(filename)
    val proto = org.cryptobiotic.protogen.DecryptionResult.decodeFromByteArray(buffer)
    return proto.import(this)
}
...
class PlaintextBallotIterator(
    ...
    override fun computeNext() {
        while (true) {
            ...
            val ballotProto = org.cryptobiotic.protogen.PlaintextBallot.decodeFromByteArray(message)
...
class EncryptedBallotIterator(
    ...
    private val filename: String,
    private val protoFilter: ((org.cryptobiotic.protogen.EncryptedBallot) -> Boolean)?,
    ...
    override fun computeNext() {
        while (true) {
            ...
            val message = readFromFile(file, length.toULong(), filename)
            val ballotProto = org.cryptobiotic.protogen.EncryptedBallot.decodeFromByteArray(message)
...
class SpoiledBallotTallyIterator(
    ...
    override fun computeNext() {
        ...
        val message = readFromFile(file, length.toULong(), filename)
        val tallyProto = org.cryptobiotic.protogen.DecryptedTallyOrBallot.decodeFromByteArray(message)
...
fun GroupContext.readTrustee(filename: String): DecryptingTrusteeIF {
    val buffer = gulp(filename)
    val trusteeProto = org.cryptobiotic.protogen.DecryptingTrustee.decodeFromByteArray(buffer)
